...
try {
  ...
  assert.deepEqual(writeHealthStatus(statusPath, ["Discovery stale"], checkedAt), {
    checkedAt: "2026-07-27T12:34:56.000Z",
    healthy: false,
  ...
  writeFileSync(statusPath, '{"checkedAt":"not-a-date","healthy":true,"problems":[]}');
  assert.equal(readHealthStatus(statusPath), null);
  writeFileSync(statusPath, '{"checkedAt":"2026-07-27T12:34:56.000Z","healthy":true,"problems":["stale"]}');
  assert.equal(readHealthStatus(statusPath), null);
